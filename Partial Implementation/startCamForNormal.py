...
while(cap.isOpened()):
  ...
  if ret == True:
    ...
    edges = cv2.Laplacian(img_gray, ddepth=-1, ksize=5, scale=1, borderType=cv2.BORDER_DEFAULT)
    edges2 = cv2.Laplacian(img_gray, ddepth=-1, ksize=5, scale=1, borderType=cv2.BORDER_DEFAULT)
